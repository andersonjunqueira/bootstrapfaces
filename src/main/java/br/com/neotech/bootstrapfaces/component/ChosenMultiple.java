...
public class ChosenMultiple extends HtmlSelectManyListbox implements IFormControl {
    ...
    public void posRender() {
        ...
        String initScript = getInitScript() != null ? getInitScript() : "";

        String chosenOptions =
            "{ no_results_text: '" + Mensagens.COMPONENTES.get("framework.chosen.semresultados") + "', " +
            "  placeholder_text_single: '" + Mensagens.COMPONENTES.get("framework.chosen.selecioneuma") + "', " +
            "  placeholder_text_multiple: '" + Mensagens.COMPONENTES.get("framework.chosen.selecionevarios") + "' }";

        String script = new StringBuilder()
            .append("$('.").append(getStyleClassId()).append("').chosen(" + chosenOptions + ");")
            .toString();
